...
client.once('ready', () => {
	...
	.then (registerEntities) // developent only!
});
...
async function registerCommands(){
	...
	const commandFiles = readdirSync(`${__dirname}/commands/`).filter(file => !file.endsWith('map'));
...
async function registerEvents(){
	const eventFiles = readdirSync(`${__dirname}/events`).filter(file => !file.endsWith('map'));
